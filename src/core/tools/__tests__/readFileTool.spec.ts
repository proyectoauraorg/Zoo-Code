...
describe("ReadFileTool", () => {
	...
	describe("batch approval flow", () => {
		it("should approve all files in batch when user clicks yes", async () => {
			const mockTask = createMockTask()
			const callbacks = createMockCallbacks()
		...
		it("should include feedback in denied result", async () => {
			const mockTask = createMockTask()
			const callbacks = createMockCallbacks()
			...
			await readFileTool.execute({ path: "secret.ts" }, mockTask as any, callbacks)

			expect(formatResponse.toolDeniedWithFeedback).toHaveBeenCalledWith("Don't read this")
		})

		it("should include feedback in approved result", async () => {
			const mockTask = createMockTask()
			const callbacks = createMockCallbacks()
			...
			expect(formatResponse.toolApprovedWithFeedback).toHaveBeenCalledWith("Go ahead")
		})
	})
