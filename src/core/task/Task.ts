...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	...
	async say(
		...
		if (this.abort) {
			throw new Error(`[ZooCode#say] task ${this.taskId}.${this.instanceId} aborted`)
		}
	...
	public async recursivelyMakeClineRequests(
		...
		while (stack.length > 0) {
			...
			if (this.abort) {
				throw new Error(`[ZooCode#recursivelyMakeZooRequests] task ${this.taskId}.${this.instanceId} aborted`)
			}
			...
			try {
				...
				if (this.abort || this.abandoned) {
					throw new Error(
							`[ZooCode#recursivelyMakeZooRequests] task ${this.taskId}.${this.instanceId} aborted`,
						)
				}
